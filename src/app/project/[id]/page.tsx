  ...
  const fetchProject = useCallback(async () => {
    try {
      ...
      if (project.name) {
        ...
        // If the project name is not the default, mark it as set
        if (project.name !== "Database Schema" && project.name !== "New Database Schema") {
          setIsProjectNameSet(true);
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.schema && data.schema.tables && data.schema.tables.length > 0) {
        ...
        // Update project name based on schema if it's still the default and hasn't been set yet
        if (!isProjectNameSet && (projectName === "Database Schema" || projectName === "New Database Schema")) {
          const newName = generateProjectName(mergedSchema);
          setProjectName(newName);
          setIsProjectNameSet(true); // Mark the project name as set
